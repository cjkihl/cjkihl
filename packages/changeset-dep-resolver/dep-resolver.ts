...
async function getNpmPackageInfo(packageName: string): Promise<PackageInfo | null> {
  try {
...
async function resolveWorkspaceDependencies(cwd: string = process.cwd()) {
      ...
      for (const [depName, depVersion] of Object.entries(deps)) {
        if (typeof depVersion === "string" && depVersion.startsWith("workspace:")) {
          ...
          if (releasingPackages.has(depName)) {
            ...
            console.log(`📦 ${packageJson.name}: ${depName}@${depVersion} → ${resolvedVersion} (from release plan)`);
          } else if (localPackages.has(depName)) {
            // Use the current local version
            resolvedVersion = localPackages.get(depName)!;
            console.log(`📦 ${packageJson.name}: ${depName}@${depVersion} → ${resolvedVersion} (local package)`);
          } else {
            // Try to get the latest version from npm
            const npmInfo = await getNpmPackageInfo(depName);
            if (npmInfo) {
              resolvedVersion = npmInfo.version;
              console.log(`📦 ${packageJson.name}: ${depName}@${depVersion} → ${resolvedVersion} (from npm)`);
            } else {
              console.warn(`⚠️  ${packageJson.name}: Could not resolve ${depName}@${depVersion} - keeping as is`);
              continue;
            }
          }
  ...
  if (modifiedPackages.size > 0) {
    ...
    return { modifiedPackages: [], backupPath: null };
}
