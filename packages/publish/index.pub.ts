...
function getPublishCommand(packageManager: string, options: PublishOptions): string {
  ...
  const command = args.length > 0 ? `${baseCommand} ${args.join(" ")}` : baseCommand;
...
async function checkNpmAuth(packageManager: string): Promise<void> {
  ...
  if (!authToken) {
    ...
    console.error(`Please set ${packageManager === "bun" ? "NPM_CONFIG_TOKEN" : "NPM_TOKEN"} environment variable`);
...
export async function publish(packagePath: string, userOptions: Partial<PublishOptions> = {}): Promise<void> {
  ...
  try {
    ...
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(() => {
        reject(new Error(`Command timed out after 5 minutes: ${publishCommand}`));
      }, 5 * 60 * 1000);
    });
    ...
    try {
      ...
      if (errorMessage.includes("E403")) {
        console.error("❌ Permission denied: You don't have permission to publish this package");
