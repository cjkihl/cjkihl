...
describe("parseExportPath", () => {
	test("parses root index file", () => {
		...
		const rel = require("node:path")
			.relative(TEST_DIR, "index.pub.ts")
			.replace(/\.pub\.(ts|tsx)$/, "");
	...
	test("parses nested index file", () => {
		...
		const rel = require("node:path")
			.relative(TEST_DIR, "src/index.pub.ts")
			.replace(/\.pub\.(ts|tsx)$/, "");
		const isIndex = rel.split("/").pop() === "index";
		const name = isIndex
			? rel.split("/").length === 1
	...
	test("parses regular file", () => {
		...
		const rel = require("node:path")
			.relative(TEST_DIR, "src/utils.pub.ts")
			.replace(/\.pub\.(ts|tsx)$/, "");
		const isIndex = rel.split("/").pop() === "index";
		const name = isIndex
			? rel.split("/").length === 1
...
describe("parseBinaryPath", () => {
	test("parses binary file path", () => {
		...
		const rel = require("node:path")
			.relative(TEST_DIR, "src/cli.bin.ts")
			.replace(/\.bin\.(ts|tsx)$/, "");
		const name = rel
			.split("/")
			.pop()
	...
	test("handles special characters in name", () => {
		...
		const rel = require("node:path")
			.relative(TEST_DIR, "src/my-cli-tool.bin.ts")
			.replace(/\.bin\.(ts|tsx)$/, "");
		const name = rel
			.split("/")
			.pop()
	...
	test("throws error for empty string", () => {
		expect(() => parseBinaryPath("", TEST_DIR)).toThrow(
			"Invalid name for binary file: ",
		);
	});
});
...
describe("generateExports", () => {
	test("generates exports configuration", () => {
		...
		const rel1 = require("node:path")
			.relative(TEST_DIR, "index.pub.ts")
			.replace(/\.pub\.(ts|tsx)$/, "");
		const rel2 = require("node:path")
			.relative(TEST_DIR, "src/utils.pub.ts")
			.replace(/\.pub\.(ts|tsx)$/, "");
		const isIndex1 = rel1.split("/").pop() === "index";
		const name1 = isIndex1
			? rel1.split("/").length === 1
				? "."
				: `./${rel1.split("/").slice(0, -1).join("/")}`
			: `./${rel1}`;
		const isIndex2 = rel2.split("/").pop() === "index";
		const name2 = isIndex2
			? rel2.split("/").length === 1
				? "."
				: `./${rel2.split("/").slice(0, -1).join("/")}`
			: `./${rel2}`;
		const expected: Record<string, { default: string; types: string }> = {};
		expected[name1] = {
			default: `./dist/${rel1}.pub.js`,
			types: `./types/${rel1}.pub.d.ts`,
		};
		expected[name2] = {
			default: `./dist/${rel2}.pub.js`,
			types: `./types/${rel2}.pub.d.ts`,
		};
	...
	test("sorts exports alphabetically", () => {
		...
		const result = generateExports(files, TEST_DIR, "dist", "types");
		const relA = require("node:path")
			.relative(TEST_DIR, "a.pub.ts")
			.replace(/\.pub\.(ts|tsx)$/, "");
		const relZ = require("node:path")
			.relative(TEST_DIR, "z.pub.ts")
			.replace(/\.pub\.(ts|tsx)$/, "");
		const isIndexA = relA.split("/").pop() === "index";
		const nameA = isIndexA
			? relA.split("/").length === 1
				? "."
				: `./${relA.split("/").slice(0, -1).join("/")}`
			: `./${relA}`;
		const isIndexZ = relZ.split("/").pop() === "index";
		const nameZ = isIndexZ
			? relZ.split("/").length === 1
				? "."
				: `./${relZ.split("/").slice(0, -1).join("/")}`
			: `./${relZ}`;
...
describe("generateBin", () => {
	test("generates bin configuration", () => {
		...
		const rel1 = require("node:path")
			.relative(TEST_DIR, "cli.bin.ts")
			.replace(/\.bin\.(ts|tsx)$/, "");
		const rel2 = require("node:path")
			.relative(TEST_DIR, "tools/helper.bin.ts")
			.replace(/\.bin\.(ts|tsx)$/, "");
		const name1 =
			rel1
				.split("/")
	...
	test("sorts bin entries alphabetically", () => {
		...
		const result = generateBin(files, TEST_DIR, "dist");
		const relA = require("node:path")
			.relative(TEST_DIR, "a.bin.ts")
			.replace(/\.bin\.(ts|tsx)$/, "");
		const relZ = require("node:path")
			.relative(TEST_DIR, "z.bin.ts")
			.replace(/\.bin\.(ts|tsx)$/, "");
		const nameA = relA
			.split("/")
			.pop()
